...
module gameLogic {
  ...
  function canMove(i: number, j: number, board: Board): number {
    var loop: ILoop = { rowFrom: 0, rowTo: 0, colFrom: 0, colTo: 0 };
    var x: number
    var y: number;
  ...
  function getWinner(board: Board, turnIndex: string): string {
    var Actr = 0, Bctr = 0;
    var i: number;
    var j: number;
  ...
  export function createMove(pawnPosition: IDelta, pawnDelta: IDelta, turnIndexBeforeMove: number, stateBeforeMove: any): IMove {
    ...
    if (turnInfo.ctr === 1) {
      ...
      if (winner === '') {
        if (turnIndexBeforeMove === 0) { newTurn = 1; newTurnInfo.ctr = 2; newTurnInfo.pawn = 'B'; }
        else if (turnIndexBeforeMove === 1) { newTurn = 0; newTurnInfo.ctr = 2; newTurnInfo.pawn = 'A'; }
      }
      else {
  ...
  export function horizontalMoveCheck(pos1: IDelta, pos2: IDelta, board: Board): boolean {

    var i: number;
    var greaterpos: IDelta;
    var lesserpos: IDelta;
  ...
  export function verticalMoveCheck(pos1: IDelta, pos2: IDelta, board: Board): boolean {
  ...
  export function diagonalMoveCheck(pos1: IDelta, pos2: IDelta, board: Board): boolean {
  ...
  export function getExampleGame(): any {
    var game = [
      {
  ...
  export function getRiddles(): any {
    var riddles = [{
      turnIndexBeforeMove: { turnIndex: 1 },
      stateBeforeMove: {
  ...
  export function createComputerMove(stateBeforeMove: any, turnIndexBeforeMove: any): IMove {
    ...
    for (i = 0; i < pawnPosList.length; i += 1) {
      ...
      while (startRow < 10 && startCol < 10) {
        if (board[startRow][startCol] === '') {
          temp = { row: startRow, col: startCol }; pawnDelList[index].push(temp); startRow += 1; startCol += 1;
        } //SE
        else { break; }
      }
      ...
      while (startRow >= 0 && startCol < 10) {
        if (board[startRow][startCol] === '') {
          temp = { row: startRow, col: startCol }; pawnDelList[index].push(temp);
    ...
    var pawnNumber: number;
    var listLen: number;
    var pawnDelNumber: number;
    var tempArray: any;
    while (1) {
      if (turnInfo.ctr === 2) {
